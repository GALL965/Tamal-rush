...
func fetch_stats():
	...
	http.request("http://localhost:8080/api/last-run")
...
func _on_request_completed(result, response_code, headers, body):
	...
	lbl_speed.text     = str(data.get("speed", 0))
	lbl_dash.text      = str(data.get("dash_power", 0))
	lbl_collected.text = str(data.get("tamales_collected", 0))
	lbl_banked.text    = str(data.get("tamales_banked", 0))
	lbl_time.text      = str(data.get("total_time", 0)) + " s"
